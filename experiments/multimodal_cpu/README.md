...
This analysis is request-independent: it reads checkpoint metadata and model
configuration, so it does not consume NPU-baseline input scale. The shared
runtime hooks are present only because all runnable branches inherit the same
instrumented source baseline.
